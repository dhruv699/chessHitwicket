...
public:
    ...
    bool IsLegalMove(int iSrcRow, int iSrcCol, int iDestRow, int iDestCol, GamePiece* GameBoard[5][5]) {
...
private:
    virtual bool AreSquaresLegal(int iSrcRow, int iSrcCol, int iDestRow, int iDestCol, GamePiece* GameBoard[5][5]) = 0;
    char mPieceColor;
};
...
private:
    ...
    bool AreSquaresLegal(int iSrcRow, int iSrcCol, int iDestRow, int iDestCol, GamePiece* GameBoard[5][5]) {
        GamePiece* qpDest = GameBoard[iDestRow][iDestCol];
        if (qpDest == 0) {
...
public:
    CBoard() {
        ...
        for (int iCol = 3; iCol < 5; ++iCol) {
            MainGameBoard[4][iCol] = new PawnPiece('B');
        }
        MainGameBoard[4][0] = new RookPiece('B');
        MainGameBoard[4][1] = new KnightPiece('B');
        MainGameBoard[4][2] = new BishopPiece('B');
  
        // Allocate and place white pieces
        for (int iCol = 3; iCol < 5; ++iCol) {
            MainGameBoard[0][iCol] = new PawnPiece('W');
        }
    ...
    void Print() {
        ...
        const int kiSquareHeight = 3;int x=0;
        for (int iRow = 0; iRow < 5*kiSquareHeight; ++iRow) {
            
            int iSquareRow = iRow/kiSquareHeight;
         
            if (iRow % 3 == 1) {
                cout << '-' << (char)('1' + 4 - iSquareRow) << '-';
            } else {
                cout << "---";
            }
           
            for (int iCol = 0; iCol < 5*kiSquareWidth; ++iCol) {
                int iSquareCol = iCol/kiSquareWidth;
                if (((iRow % 3) == 1) && ((iCol % 4) == 1 || (iCol % 4) == 2) && MainGameBoard[6-iSquareRow][iSquareCol] != 0) {
                    if ((iCol % 4) == 1) {
                        cout << MainGameBoard[4-iSquareRow][iSquareCol]->GetColor();
                    } else {
                        cout << MainGameBoard[4-iSquareRow][iSquareCol]->GetPiece();
                    }
                } else {
        ...
        for (int iRow = 0; iRow < kiSquareHeight; ++iRow) {
            if (iRow % 3 == 1) {
                ...
                for (int iCol = 1; iCol < 6*kiSquareWidth; ++iCol) {
                    cout << '-';
                }
                cout << endl;
            }
        ...
        for (int iRow = 0; iRow < 5; ++iRow) {
            for (int iCol = 0; iCol < 5; ++iCol) {
                if (MainGameBoard[iRow][iCol] != 0) {
                    ...
                    if (MainGameBoard[iRow][iCol]->GetColor() == PieceColor) {
                        for (int iMoveRow = 0; iMoveRow < 5; ++iMoveRow) {
                            for (int iMoveCol = 0; iMoveCol < 5; ++iMoveCol) {
...
public:
    ...
    void GetNextMove(GamePiece* GameBoard[5][5]) {
        using namespace std;
        bool bValidMove     = false;
        do {
            system ("clear");
            cout<<endl<<endl<<"          Welcome to the Game Developed by DhruvCorporations "<<endl<<endl<<endl;
